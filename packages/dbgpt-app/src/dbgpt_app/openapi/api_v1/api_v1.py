...
from dbgpt_serve.utils.auth import (
    HEADER_AUTH_TOKEN_KEY,
    UserRequest,
    get_user_from_headers,
)
from dbgpt_serve.utils.user_auth import get_user_auth_store
...
@router.post("/v1/user/register", response_model=Result[LoginUserResponse])
async def register_user(body: RegisterUserRequest = Body()):
    username = body.username.strip()
    password = body.password
    confirm_password = body.confirm_password
    if not username:
        return Result.failed(code="E0201", msg="Username is required.")
    if password != confirm_password:
        return Result.failed(code="E0202", msg="Passwords do not match.")
    try:
        auth_store = get_user_auth_store()
        auth_store.register_user(username, password)
        user, token = auth_store.login(username, password)
        return Result.succ(
            LoginUserResponse(
                token=token,
                user_id=user.user_id or username,
                user_name=user.user_name or username,
                nick_name=user.nick_name or username,
                role=user.role or "normal",
            )
        )
    except ValueError as e:
        return Result.failed(code="E0203", msg=str(e))


@router.post("/v1/user/login", response_model=Result[LoginUserResponse])
async def login_user(body: LoginUserRequest = Body()):
    username = body.username.strip()
    password = body.password
    if not username or not password:
        return Result.failed(code="E0204", msg="Username and password are required.")
    try:
        user, token = get_user_auth_store().login(username, password)
        return Result.succ(
            LoginUserResponse(
                token=token,
                user_id=user.user_id or username,
        ...
        return Result.failed(code="E0205", msg=str(e))


@router.get("/v1/user/me", response_model=Result[UserRequest])
async def get_current_user(user_token: UserRequest = Depends(get_user_from_headers)):
    return Result.succ(user_token)


@router.post("/v1/user/logout", response_model=Result[bool])
async def logout_user(
    auth_token: Optional[str] = Header(None, alias=HEADER_AUTH_TOKEN_KEY),
    user_token: UserRequest = Depends(get_user_from_headers),
):
    if auth_token and user_token.user_id:
        get_user_auth_store().logout(user_token.user_id, auth_token)
    return Result.succ(True)
