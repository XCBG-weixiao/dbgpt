...
def _ensure_owned_conversation(
    service: Service, conv_uid: str, user_token: UserRequest
) -> None:
    conv = service.get(ServeRequest(conv_uid=conv_uid, user_name=user_token.user_id))
    if conv is None:
...
async def query(
    ...
    """
    request.user_name = user_token.user_id
...
async def dialogue_new(
    ...
    user_name = user_token.user_id or user_name or user_id
...
async def delete(
    ...
    Args:
        ...
        service (Service): The service
    """
    _ensure_owned_conversation(service, con_uid, user_token)
    service.delete(ServeRequest(conv_uid=con_uid, user_name=user_token.user_id))
...
async def clear(
    ...
    """
    _ensure_owned_conversation(service, con_uid, user_token)
    service.clear(ServeRequest(conv_uid=con_uid, user_name=user_token.user_id))
    return Result.succ(None)
...
async def get_history_messages(
    ...
    return Result.succ(
        service.get_history_messages(
            ServeRequest(conv_uid=con_uid, user_name=user_token.user_id)
        )
    )
